...
while len(candsuggs)>0:
    ...
    members=[]
    new_members=[cursugg]
    while(len(members) < len(new_members)): # 이 루프에서 cursugg랑 연결되는 애들은 다 나옴
        members=new_members
        right_simscores=[]
        left_simscores=[]
        for sugg in members: 
    ...
    #members에는 cursugg랑 연결된 모든 suggestion이 list로 담겨있음 --> 이중 가장 popular 한 것ㅇ르 대표로 선정 
    #print(new_members)
    count=0
    ...
    thisFS.save()
    #print(thisFS, maxmem, count ,new_members)
